...
# All inputs and the start button must be defined here first.
with st.sidebar:
    st.header("⚙️ Configuration")
    alsoasked_key = st.text_input("AlsoAsked API Key", type="password")
    gemini_key = st.text_input("Gemini API Key", type="password")

    st.header("Advanced Mapping")
    ...
    # This variable is now guaranteed to be defined before line 65
    start_button = st.button("Start Full Analysis")

# --- MAIN WORKFLOW ---
# This block only executes after the sidebar has been processed.
if start_button:
    ...
    with st.spinner("Running full analysis with Gemini 2.0..."):
        ...
        for keyword in keywords:
            for persona in active_personas:
                ...
                # Gemini Variations with search metadata using the new SDK
